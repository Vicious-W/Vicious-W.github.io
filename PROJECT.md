...
- 实现和审查使用全新进程严格串行，审查阶段只读；同一任务内各角色分别恢复自己
  的专属会话，角色间绝不共享；
...
在现有完整人工控制基础上恢复全自动运行，形成 `AUTO` 与 `MANUAL` 双模式。普通
用户第一次在控制台以外与场景交互时进入 AUTO，完整经历启动、低功率、历史脉冲、
脉冲后传热和 250 kW 平衡；直接操作控制台的用户进入 MANUAL。人工接管自动程序时
继承当前棒位、功率、温度、流量、水体和玻璃状态，不能重置或同时运行两套控制器。
