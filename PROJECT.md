...
在已经接受的反应堆池、格栅玻璃及 AUTO/MANUAL 基线上完成 SOURCE 第二阶段：

- 参考 Pavia 与不冲突的正式研究堆资料，重建完整实验室和可观察的地下设备层；
- 建立可进入水下、堆芯附近和地下设备区的轨道/自由飞行相机；
- 重做清澈轻水、焦散、水下光学，以及由功率驱动的局部切伦科夫体积/粒子表现；
- 保留现有 MANUAL 控制台，在旁边增加共享同一反应堆状态的 AUTO 控制台；
- 墙壁和天花板使用固定玻璃砖，地板使用透明结构层承托的动态玻璃砖；
- 格栅和地板动态玻璃在抓取时由鼠标水平移动、`W/S` 上下、`A/D` 水平偏航，
  不产生随机旋转。

正式目标与验收见 `PROJECT_SPEC.md`；详细工程基线见
