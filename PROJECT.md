...
- `src/scenes/reactor/physicalScene.js`：唯一页面场景，负责渲染、玻璃刚体、格栅物理、
  轻水/损伤耦合、控制台射线拾取、首次交互的 AUTO/MANUAL 分流、右键轨道相机和滚轮缩放；
- `src/scenes/reactor/sessionController.js`：AUTO 与 MANUAL 共用的反应堆模型，持有唯一
  控制权状态，提供启动、SCRAM、模式、冷却泵、三根控制棒和脉冲命令，并积分点堆动力学、
  温度反馈与两节点热工状态；
- `src/scenes/reactor/autoProgram.js`：AUTO 连续运行调度器，用与人工完全相同的设备指令
  和联锁依次完成联锁复位、辅助设备就绪、低功率临界、脉冲就绪、历史脉冲、脉冲后传热、
  稳态功率提升和 250 kW 平衡；
...
- `tests/run.mjs`：`npm test` 的 node 逻辑测试（会话复位、控制权分流与接管、AUTO 全程、
  脉冲联锁、帧率无关性、玻璃损伤与碎片、池体/轻水/厂房不变量）。

当前反应堆参考 Pavia TRIGA Mark II 研究堆，堆芯采用中央 A 位辐照套管 + B–F 90 个
元件位置的构型，三根控制棒（SHIM/C 环、TRANS/D 环、REG/E 环）独立驱动。

页面加载后保持联锁复位。第一次有效交互解锁音频与场景时钟，并按命中对象决定控制权：
控制台热点以外的指针/滚轮/键盘交互进入 `AUTO`，由调度器完整走完联锁复位 → 辅助设备
就绪 → 低功率临界 → 脉冲就绪 → 历史脉冲 → 脉冲后传热 → 稳态功率提升 → 250 kW 平衡；
直接操作三维控制台热点进入 `MANUAL`，由用户启动、操作冷却泵、连续提插控制棒、切换
运行/脉冲模式、触发脉冲或 SCRAM。AUTO 运行中的任一人工指令原位接管并继承当前棒位、
功率、温度、流量、水体和玻璃状态；从 MANUAL 重新进入完整 AUTO 只能在 SCRAM/安全停堆
后由控制台上的无文字 AUTO 方钮触发。任意时刻只有一个控制权所有者，两种模式共用同一套
命令、联锁、动力学和热工状态。棒位、反应性、功率、燃料/池水温度、流量、轻水、厂房
状态和玻璃共享同一组物理状态。

功率代理 `1.0` 代表 250 kW 稳态，脉冲通道 `1.0` 代表 250 MW 历史脉冲峰值；脉冲前功率
联锁按资料取 100 W 代理（`4e-4`），因此无法从满功率直接点火。
...
- Agent 身份只有 `GENERAL`、`IMPLEMENTER` 与 `REVIEWER`；GENERAL 同时承担普通
  协作、控制面维护与轮转监督，不再设置独立 MONITOR 身份；
- 角色不与 Claude Code 或 Codex 绑定，父脚本可为每个角色指定执行器、模型和
  effort；
- 当前默认配置仍是 Claude Code 实现、Codex 审查；
- 实现和审查使用全新进程严格串行，审查阶段只读；同一任务内各角色分别恢复自己
  的专属会话，角色间绝不共享；
- `agent-cycle.sh` 管理有界轮回；一次轮回固定为一次实现，审查只出现在相邻两次
...
当前任务只改进 SOURCE 观察相机，使所有者能够稳定、细致地检阅已经实现的实验室：

- 右键围绕画面中心锁定的场景焦点缓慢旋转；
- 滚轮经过归一化和阻尼后连续推进，不从一端跳到另一端；
- 方向键移动画面中心，并在失焦后安全停止；
- 抓取玻璃时暂停相机输入，保留 `W/S` 上下和 `A/D` 水平偏航；
- 保持水下、堆芯附近和地下设备层可达，不复位会话或推动物理对象。

本轮不修改实验室、反应堆或外部世界。完成后由所有者先检阅当前实验室，再根据实际
观察提出实验室改进意见；园区、外部环境、蓝天和动态云层只能在新的后续任务中定义。

正式目标与验收见 `PROJECT_SPEC.md`，执行切片见 `.agent/next-task.md`；相机工程
基线见 `docs/engineering/SOURCE_LAB_OPTICS.md`。
